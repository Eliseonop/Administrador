...
import { eliminarProducto } from "../service/VentasService";
import {
  Card,
  Container,
  Button,
...
const NotifyCard = ({ item,obtenerVentas }) => {
  const {
    nombreCompleto,
    telefono,
    email,
  ...
  const manejarEliminar = async () => {
    ...
    if (accionUsuario.isConfirmed) {
      try {
        ...
        Swal.fire("   Pedido!", "En camino ", "success");
    ...
    <>
      <div className="card mb-4">
        <div className="d-flex m-1 ">
          <span className="fw-bold">Nuevo Pedido:</span>{" "}
          <span className="fst-italic ms-3"> {fecha} </span>{" "}
        </div>
        <hr />
        <Row>
          <Col className=" d-grid col-lg-3 ">
            <div className="d-flex flex-column  flex-lg-column flex-sm-row   d-grid  ">
              ...
              <Button
                className=" btn col-lg-6  btn-success m-2 w-75   "
